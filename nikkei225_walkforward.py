...
class EmaCrossRsiStrategy(bt.Strategy):
    params = (
        ...
        ('rsi_period', 14), ('rsi_upper', 55), ('rsi_lower', 45),
        ('position_pct', 0.25),
    )
    ...
    def next(self):
        ...
        # ロングポジション保有中の場合
            ...
            # 手仕舞い条件 (デッドクロス or RSI低下)
            if self.crossover < 0 or self.rsi[0] <= self.p.rsi_lower:
                self.order = self.close()
                
        # ショートポジション保有中の場合
        elif self.position.size < 0:
            # 手仕舞い条件 (ゴールデンクロス or RSI上昇)
            if self.crossover > 0 or self.rsi[0] >= self.p.rsi_upper:
                self.order = self.close()
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            # 決済取引だった場合のみ、勝敗をカウントする
            # (order.executed.size と order.executed.price が記録される)
            # order.isbuy() は買い注文(新規ロング or ショート決済)
            # order.issell() は売り注文(新規ショート or ロング決済)
            
            # 決済の場合 (保有ポジションが0になった、または減った場合の大雑把な判定)
            # 厳密な勝敗はトレード機能 (notify_trade) で判定する方が正確
            pass
            
        self.order = None

    def notify_trade(self, trade):
        """1つの取引（エントリーから決済まで）が完了したときに呼ばれる"""
        if not trade.isclosed:
            return
            
        self.trade_count += 1
        if trade.pnl > 0:  # 利益(pnl)がプラスなら勝ち
            self.win_count += 1


def run_bt(df: pd.DataFrame, initial_cash: float) -> dict:
    """DataFrameでバックテストを実行し結果を返す（出力抑制）"""
...
if __name__ == '__main__':
    ...
    PERIOD       = "60d"
    TRAIN_DAYS   = 40
    MAX_WORKERS  = 8   # 並列ダウンロード数（増やすとyfinanceのレート制限に注意）

    print("=" * 70)
    print("📊 日経225 ウォークフォワードバックテスト")
    ...
    print("\n" + "-" * 70)
    print(f"  選定→検証でプラス銘柄: {winners} / {len(df_test)} 銘柄")
    print(f"  全選定銘柄 合計損益:    {total:+,}円")
    ...
    csv_path = "nikkei225_walkforward.csv"
